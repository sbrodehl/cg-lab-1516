...
typedef std::vector<QVector2D *> Spline;
typedef std::set<Line *>::iterator LineIt;
...
std::vector<ParameterTriangle *> TriangulationAlgorithm::triangulate(std::vector<Spline> polygon) {
    for (Spline spline : polygon) {
        ...
        for (int i = 0; i < spline.size(); i++) {
            lines.push_back(new Line(spline[i], spline[(i + 1) % spline.size()]));
            vertices.push_back(new Vertex(*spline[i]));
        }
    }
    ...
    makeMonotone();
    createPolygons();
    for (std::vector<Vertex *> p : polygons) {
...
std::vector<ParameterTriangle *> TriangulationAlgorithm::createParameterTriangles(
        std::vector<std::vector<Vertex *>> polygons) {
    std::vector<ParameterTriangle *> triangles;

    for (std::vector<Vertex *> polygon : polygons) {
        ParameterTriangle *t = new ParameterTriangle(*polygon[0]->getPoint(),
                                                     *polygon[1]->getPoint(),
                                                     *polygon[2]->getPoint());
...
void TriangulationAlgorithm::triangulateMonotone(std::vector<Vertex *> polygon) {
    ...
    while (pq.size() > 1) {
        ...
        if (((getIndex(left, v) != -1) && (getIndex(right, top) != -1)) ||
            ((getIndex(right, v) != -1) && (getIndex(left, top) != -1))) {
            while (!stack.empty()) {
    ...
    for (std::pair<Vertex *, std::vector<Edge *>> evpair : graph) {
        ...
        for (Edge *e : ev) {
            ...
            do {
                ...
                // get most left edge
                Edge *out = outEdges[0];
                double min_angle = start->angleBetween2Lines(*actual->getv1()->getPoint(),
                                                             *actual->getv2()->getPoint(),
                                                             *out->getv2()->getPoint());
                for (Edge *o : outEdges) {
                    if (o->visited) {
                        continue;
                    ...
                    double angle = start->angleBetween2Lines(*actual->getv1()->getPoint(),
                                                             *actual->getv2()->getPoint(),
                                                             *o->getv2()->getPoint());
                    if (angle < min_angle) {
                        min_angle = angle;
...
void TriangulationAlgorithm::handleSplit(Vertex *p) {
    LineIt it = SL.lower_bound(new Line(p->getPoint(), p->getPoint()));
    Line *e = *it;
    Vertex *p2 = helpers[e];
    lines.push_back(new Line(p->getPoint(), p2->getPoint()));
    graph[p].push_back(new Edge(p, p2));
...
void TriangulationAlgorithm::handleRegular(Vertex *p) {
    if (p->getNeighbor(0)->y() > p->y()
        || (p->getNeighbor(0)->y() == p->y() && p->getNeighbor(0)->x() < p->x())) {
        int idx = getIndex(vertices, p->getNeighbor(0));
        Line *e = lines[idx];
